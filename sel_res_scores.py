...
#Selectively scores certain residues of protein

def main():
  pose = Pose()
  str_path = '/home/achitturi/Desktop/' + sys.argv[1] + "-structures/" #finds given filepath to search for PDB files
  os.chdir(str_path)
  ...
  for pdb_struct in pdb_struct_list: # pdb_struct = something like './PP2C_chlorophyll_default_XXXX.pdb'
    ...
    scorefxn = get_fa_scorefxn() #Creates default scorefunction
    pose = Pose()
    pose = pose_from_pdb(pdb_path)
    print scorefxn(pose)
    scorefxn.show(pose) #sanity check

    #List of selected residues to score
    ...
    score = 0
    for i in A_res_list: #Score the residues in receptor binding site
      score = score + pose.energies().residue_total_energy(int(pose.pdb_info().pdb2pose('A',i)))
    for j in B_res_list: #Score the phosphorylated peptide
